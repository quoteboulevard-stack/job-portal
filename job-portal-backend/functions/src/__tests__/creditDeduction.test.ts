...
jest.mock('firebase-functions', () => ({
  runWith: () => ({ firestore: { document: () => ({ onUpdate: (h: Function) => h }) } }),
...
const mockRunTx  = jest.fn().mockImplementation((cb: Function) => cb(mockTx));
const mockDb     = { collection: mockColl, runTransaction: mockRunTx };
jest.mock('../shared/firebaseAdmin', () => ({ getFirestore: () => mockDb }));

import { deductCredit } from '../credits/deductCredit';

const makeChange = (before: object, after: object) => ({
  before: { data: () => before },
  after:  { data: () => after, ref: { update: mockUpdate } },
});
const ctx = (messageId = 'msg1') => ({ params: { messageId } });
const userSnap = (balance: number, exists = true) => ({
...
describe('deductCredit', () => {
  test('ignores non sent→seen transitions', async () => {
    await (deductCredit as Function)(makeChange({ status: 'sent' }, { status: 'expired', fromUserId: 'u1' }), ctx());
    expect(mockRunTx).not.toHaveBeenCalled();
  });

  test('ignores already-deducted messages', async () => {
    await (deductCredit as Function)(
      makeChange({ status: 'sent' }, { status: 'seen', fromUserId: 'u1', creditDeducted: true }), ctx()
    );
    expect(mockRunTx).not.toHaveBeenCalled();
  });

  test('ignores reverse transition (seen→sent)', async () => {
    await (deductCredit as Function)(makeChange({ status: 'seen' }, { status: 'sent', fromUserId: 'u1' }), ctx());
    expect(mockRunTx).not.toHaveBeenCalled();
  });

  test('returns early when fromUserId is missing', async () => {
    await (deductCredit as Function)(makeChange({ status: 'sent' }, { status: 'seen' }), ctx());
    expect(mockRunTx).not.toHaveBeenCalled();
  });
  ...
  test('marks creditError on insufficient credits (balance 0)', async () => {
    ...
    await (deductCredit as Function)(
      makeChange({ status: 'sent' }, { status: 'seen', fromUserId: 'u1' }), ctx()
    );
    expect(mockUpdate).toHaveBeenCalledWith(
      expect.objectContaining({ creditDeducted: false, creditError: 'insufficient_credits' })
    );
  ...
  test('throws when user document not found (triggers retry)', async () => {
    ...
    await expect(
      (deductCredit as Function)(makeChange({ status: 'sent' }, { status: 'seen', fromUserId: 'u1' }), ctx())
  ...
  test('deducts credit and logs transaction on success', async () => {
    ...
    expect(mockTx.update).toHaveBeenCalledWith(
      expect.anything(), expect.objectContaining({ balance: 2 })
    );
    expect(mockTx.set).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ type: 'deduction', amount: 1, balanceAfter: 2, reason: 'message_viewed' })
    );
    expect(mockTx.update).toHaveBeenCalledWith(
      expect.anything(), expect.objectContaining({ creditDeducted: true })
    );
  });
  ...
  test('deducts correctly when balance is exactly 1', async () => {
    ...
    expect(mockTx.update).toHaveBeenCalledWith(
      expect.anything(), expect.objectContaining({ balance: 0 })
    );
  });
});
